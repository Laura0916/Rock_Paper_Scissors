//Get to DOM elements
const gameContainer = document.querySelector(".container"),
userResult = document.querySelector(".user_result img"),
cpuResult = document.querySelector(".cpu_result img"),
result = document.querySelector(".result"),
optionImages = document.querySelectorAll(".option_image");

const images = ["img/rock.png", "img/paper.png", "img/scissor.png"];

let cycleInterval = null; 

//Loop through each option image element
optionImages.forEach((image,index) =>{
    image.addEventListener("click", (e) => {

        //Block clicks while animation is running
        if(gameContainer.classList.contains("start")) return;

        image.classList.add("active");

        result.textContent = "Wait..."

        //Remove active from others
        optionImages.forEach((image2, index2) => {
            index !== index2 && image2.classList.remove("active");
        });

        //Start state
        gameContainer.classList.add("start");
        userResult.src = cpuResult.src ="img/rock.png";

        //Clear previous cycle
        if(cycleInterval){
            clearInterval(cycleInterval);
            cycleInterval=null;
        }

        //Cycling CPU and User images during shake
        let cycleIndex=0;
        cycleInterval = setInterval(() => {
            userResult.src = images[cycleIndex];
            cpuResult.src = images[cycleIndex];
            cycleIndex = (cycleIndex + 1) % images.length;
        }, 120);

        //Timeout to delay the result calculation
        let time = setTimeout(() => {
            clearInterval(cycleInterval);
            cycleInterval= null;

        gameContainer.classList.remove("start");

        //Get the source of the clicked option image
        let imageSrc = e.target.querySelector("img").src;
        //  Set the user image to the clicked option image
        userResult.src = imageSrc;

        //Generate a random number between 0 and 2
        let randomNumber = Math.floor(Math.random() * 3);
        //Create an array of CPU image options
        let cpuImages = ["img/rock.png", "img/paper.png", "img/scissor.png"];
        //Set the CPU image to a random option from the array
        cpuResult.src = cpuImages[randomNumber];

        //Assign a letter value to the CPU option (R for rock , P for paper, S for scissors)
        let cpuValue = ["R","P","S"][randomNumber];
        //Assign a letter value to the clicked option (based on index)
        let userValue = ["R","P","S"][index];

        //Object with all possible outcomes
        let outcomes={
            RR: "Draw",
            RP: "Cpu",
            RS: "You",
            PP: "Draw",
            PR: "You",
            PS: "Cpu",
            SS: "Draw",
            SR: "Cpu",
            SP: "You",
        };

        //Look up the outcome value based on user and CPU options
        let outComeValue = outcomes [userValue + cpuValue];

        //Display the result 
        result.textContent = userValue === cpuValue ? "Draw!" : `${outComeValue} Won!!`;
        },2500)
        
    });
} );